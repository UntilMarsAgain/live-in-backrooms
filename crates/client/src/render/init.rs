...
use lbr_shared::core::camera::CameraUniform;
use lbr_shared::core::mesh::Vertex;
use lbr_shared::core::texture::Texture;
use crate::render::debug::LineGizmos;
use crate::render::environment::EnvironmentResources;
use crate::render::uniform::{LIGHT_CAPACITY, LightCountUniform, LightUniform, ObjectData};
use crate::render::{BlitResources, DisplayHandle, HDR_FORMAT, Renderer};

/// 网格顶点缓冲布局：描述 wgpu 如何从 [`Vertex`] 读取数据。
///
/// 顶点布局是**客户端渲染概念**（服务端只消费 `Mesh::bounds()` 做碰撞），
/// 因此放在这里而不是 shared 层；`Vertex` 的字段顺序/偏移即缓冲区布局。
pub(super) fn mesh_vertex_layout() -> wgpu::VertexBufferLayout<'static> {
...
impl Renderer {
    ...
    /// 底层设备克隆（供 [`AssetManager`](crate::render::AssetManager)
