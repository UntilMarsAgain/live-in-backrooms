...
use lbr_shared::core::camera::{Camera, CameraUniform};
use crate::render::AssetManager;
use crate::render::init::{create_depth_texture, create_hdr_texture};
use crate::render::uniform::{
    LIGHT_CAPACITY, LightCountUniform, ObjectData, collect_lights,
};
use crate::render::{CLEAR_COLOR, Renderer};
...
impl Renderer {
    ...
    pub fn render(
        ...
        let lights = collect_lights(tree, camera.position());
        ...
        if tree.object_count() > 0 {
            ...
            let mut bytes = vec![0u8; tree.object_count() * entry_size];
            for (i, (key, _object)) in tree.objects().enumerate() {
                let model = tree
                    .world_transform(key)
...

        {
            ...
            // ---- Pass 1：场景 pass，渲染到 HDR 中间目标 ----
            {
                ...
                for (i, (_, object)) in tree.objects().enumerate() {
