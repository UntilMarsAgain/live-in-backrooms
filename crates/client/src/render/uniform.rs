...
/// 每物体数据：模型矩阵 + 法线矩阵（逆转置，正确处理非等比缩放）+ 材质参数。
///
/// 全部物体一个 storage 数组（与 WGSL `ObjectData` 对齐），按实例索引取；
/// 相比动态 uniform 的逐物体绑定，消除了设备对齐带来的步长浪费。
#[repr(C)]
...
mod tests {
    ...
    use lbr_shared::scene::{Scene, SceneObject, SceneObjectKind};
    use glam::Quat;
