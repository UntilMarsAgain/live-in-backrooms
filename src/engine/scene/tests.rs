...
fn point_inside_uses_world_aabb() {
    ...
    assert!(scene.point_inside(&MeshView::new(&assets), cube, Vec3::new(2.5, 0.5, 0.5))); // 边界上
    assert!(!scene.point_inside(&MeshView::new(&assets), cube, Vec3::new(3.0, 0.0, 0.0)));
    // 非网格节点没有包围盒，任何点都不在内。
    let empty = scene.add_object(SceneObject::new(
        SceneObjectKind::Empty,
        Transform::IDENTITY,
    ));
    assert!(!scene.point_inside(&MeshView::new(&assets), empty, Vec3::ZERO));
}
...
fn objects_collide_uses_world_aabb() {
    ...
    assert!(
        scene.objects_collide(&MeshView::new(&assets), a, b),
        "中心距 0.5 < 1 应相交"
    );
    assert!(!scene.objects_collide(&MeshView::new(&assets), a, c), "中心距 2 > 1 应分离");
    // 已删除的句柄不参与碰撞。
    scene.remove_object(c);
    assert!(!scene.objects_collide(&MeshView::new(&assets), a, c));
}
...
fn collides_with_external_probe() {
    ...
    assert_eq!(
        scene.collides_with(&MeshView::new(&assets), &player, player_box, &[]),
        Some(cube)
    );

    // 远离时无碰撞。
    let away = Transform::new(Vec3::new(2.0, 0.0, 0.0), Quat::IDENTITY, Vec3::ONE);
    assert_eq!(scene.collides_with(&MeshView::new(&assets), &away, player_box, &[]), None);
    ...
    assert_eq!(
        scene.collides_with(&MeshView::new(&assets), &rotated, player_box, &[]),
        Some(cube)
    );

    // 排除后跳过该物体。
    assert_eq!(
        scene.collides_with(&MeshView::new(&assets), &player, player_box, &[cube]),
        None
    );
}
