...
//!   （[`GcPolicy::should_keep`]）。两侧的 `gc()` 都只是拿"现在"和条目
//!   记录喂给这个函数，算法本身只在这里改。
...
#[derive(Debug, Clone, Copy)]
pub struct GcPolicy {
    /// 最近使用窗口（真实时长）：`now - last_used > stale_window` 的条目淘汰。
...
impl Default for GcPolicy {
    fn default() -> Self {
        Self {
            // 10 秒未取用才淘汰：与 GC 调用频率（秒级）解耦，取用计数
            // 不再是时间单位。渲染每帧取用的资源永远新鲜。
...
impl GcPolicy {
    /// **通用淘汰判定**：给定一条目的记录与"现在"（真实时间），
    /// 返回是否保留。两侧 `gc()` 只调用这一个函数决定去留——调算法就改这里。
    ///
    /// 当前规则：被 pin 的条目保留（除非 [`Self::evict_pinned`]）；否则
    /// 窗口内（`now - last_used <= stale_window`）保留，超窗淘汰。
    pub fn should_keep(&self, info: &GcInfo, now: Instant) -> bool {
        let pinned_kept = info.pins > 0 && !self.evict_pinned;
        if self.ignore_stale_window {
            // 全量清扫：不管最近使用，只有 pin 保护生效。
            return pinned_kept;
        }
        let idle = now.saturating_duration_since(info.last_used);
        let fresh = idle <= self.stale_window;
        pinned_kept || fresh
    }
}
