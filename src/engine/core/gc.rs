...
mod tests {
    ...
    #[test]
    fn fresh_entry_kept_within_window() {
        let policy = GcPolicy {
            stale_window: 10,
            evict_pinned: false,
            ignore_stale_window: false,
    ...
    #[test]
    fn pinned_entry_kept_unless_evict_pinned() {
        let protect = GcPolicy {
            stale_window: 0,
            evict_pinned: false,
            ignore_stale_window: false,
        };
    ...
    #[test]
    fn ignore_stale_window_releases_all_unpinned() {
        let sweep = GcPolicy {
            stale_window: 100, // 窗口再大也无效
            evict_pinned: false,
